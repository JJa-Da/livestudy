...
import matplotlib
matplotlib.use('Agg')  # Set backend before importing pyplot to avoid tkinter error
...
# Models to evaluate
MODELS_TO_EVALUATE = ['lightgbm'] # 'random_forest', 'xgboost', 
...
class EnhancedMLPipeline:
    ...
    def run_grid_search(self):
        ...
        with tqdm(total=total_combinations, desc="Grid Search Progress") as pbar:
            for model_name in MODELS_TO_EVALUATE:
                ...
                model = self._run_grid_search_for_model(
                    model_name, 
                    X_train, X_test, 
                    features, 
                    pbar
                ...
                # Track best model based on latest run
                if self.tracker.runs:
                    latest_run = self.tracker.runs[-1]
                    if 'test_mae' in latest_run and latest_run['test_mae'] < best_score:
                        best_score = latest_run['test_mae']
                        self.best_model = model
                        logger.info(f"New best model found: {model_name} with MAE: {best_score:.4f}")
    ...
    def _run_grid_search_for_model(self, 
                                   ...
                                   features: List[str],
                                   pbar: tqdm):
        """Run grid search for a specific model and return the best model"""
    ...
    def save_all_logs(self):
        ...
        if self.best_model is not None:
            ...
            # Also save scaler for later use
