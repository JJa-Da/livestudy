...
def plot_dataframe(df: pd.DataFrame, color_column: str = None, save_dir: str = None) -> None:
    """
    ...
    df : pandas.DataFrame
        The DataFrame containing the data.
    color_column : str, optional
        Optional column name to use for color (grouping).
    ...
    for column in df.columns:
        ...
        if save_dir is not None:
            file_path = os.path.join(save_dir, f"{column}_plot.html")
